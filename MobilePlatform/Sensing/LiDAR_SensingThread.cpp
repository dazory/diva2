...
// void LiDAR_SensingThread::run(void *context, mutex &m, float leaf_size)
// {
//     zmq::socket_t socketLidar(*(zmq::context_t*)context, ZMQ_PUB);
//     socketLidar.bind(protocol::SENSING_PUB);
//     socketLidar.connect("tcp://165.246.39.124:5564");
//     Timestamp ts;
//     // <make csv file>
//     string path = "/LiDAR/i30_LiDAR_ts_" + ts.getMilliTime() + ".csv";
//     fstream dataFile;
...
//                         ts->tm_year + 1900, ts->tm_mon + 1, ts->tm_mday, ts->tm_hour, ts->tm_min, ts->tm_sec, msc);
//                 string ttt(cSn);
//                 string name = "/home/diva2/build/MobilePlatform/Sensing/LiDAR/LiDAR_" + ttt + ".pcd";
//                 pcl::io::savePCDFile(name, *mLiDAR_Sensing.cloud, true);
...
//                 memcpy((void *)zmqData.data(), data, data_len);
//                 m.lock();
//                 s_send_idx(socketLidar, SENSOR_LIDAR);
//                 s_send(socketLidar, zmqData);
//                 m.unlock();
//                 printf("[MobilePlatform/Sensing/LiDARSensingThread] Complete to send to PUB Socket\n");
