...
void LiDAR_Sensing::WritePCD_proto(std::vector<double> xyz_lut, uint8_t *buf, sensors::Lidar &pLiDAR)
{
    ...
    for (int icol = 0; icol < OS1::columns_per_buffer; icol++)
    {
        ...
        for (uint8_t ipx = 0; ipx < H; ipx++)
        {
            ...
            LiDAR_point.x = r  * 0.01f * xyz_lut[ind + 0]; //leaf size 설정 -> downsampling 0.001f(787kB) 5****
            LiDAR_point.y = r  * 0.01f * xyz_lut[ind + 1]; //0.01f -> 1cm (787kB) tkdlwmsms 27***
            LiDAR_point.z = r  * 0.01f * xyz_lut[ind + 2];
            // cout<<"Output size" <<
            if (getCount() > W * H)
            {
                break;
            ...
            // cout<<"0.1f size: "<<cloud->size()<<endl;
            addCount();
        }
    }
}
